//将之前的webpack.config.js的配置文件分离 只是基础配置文件 最后执行运行或打包命令的时候需要将配置文件合并 需要下载一个合并插件
...
module.exports = {
    ...
    output:{//出口文件（打包后形成的文件）
        ...
        // publicPath:'./'//每次打包的文件后的文件路径都在dist文件夹下 这个是可以配置的
    },
    //vue-loader15版本以上的需要配置这句话
    //这里是配置plugin插件的地方
    plugins: [   //配置插件的节点，所有插件都要在这里配置
        ...
        new HtmlWebpackPlugin({
          template:'index.html'//以index.html为模板生成新的index.html
        }),
    ...
    //配置node本地服务器 下载指令 cnpm install webpack-dev-server@2.9.1 --save-dev
    // devServer:{
